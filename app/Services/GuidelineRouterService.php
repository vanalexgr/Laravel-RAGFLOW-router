...
PROMPT;
    ...
    public function selectAndExpand(string $routingQuery, int $maxGuidelines = 3, ?string $expansionQuery = null, ?array $documentAnalysis = null): array
    {
        ...
        try {
            ...
            $llmSelected = [];
            $routingResponse = $responses['routing'] ?? null;
            if ($this->isSuccessfulHttpResponse($routingResponse)) {
                $content = $routingResponse->json('choices.0.message.content', '');
                $llmSelected = $this->parseResponse($content);
            } elseif ($routingResponse instanceof \Throwable) {
                $log->warning('[PARALLEL LLM] Routing request failed; using deterministic fallback', [
                    'error' => $routingResponse->getMessage(),
                ]);
            }
    ...
    protected function isSuccessfulHttpResponse(mixed $response): bool
    {
